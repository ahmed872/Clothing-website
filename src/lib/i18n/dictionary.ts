import type { Locale } from './locales';

/**
 * Static storefront chrome copy — everything that isn't data (a category
 * name, a product name) but still needs to read naturally in both
 * languages: nav labels, empty/error states, filter/sort controls, and so
 * on. Data-bearing components keep the existing P02 pattern of Arabic-
 * default props (`locale?: Locale = 'ar'`); this dictionary is for the
 * storefront-specific chrome those components don't cover.
 */
export const dictionary = {
  ar: {
    /** The storefront footer. */
    footerSection: {
      contact: 'تواصل معنا',
      whatsapp: 'واتساب',
      email: 'البريد الإلكتروني',
      phone: 'الهاتف',
      address: 'العنوان',
    },
    nav: {
      home: 'الرئيسية',
      allCategories: 'كل الفئات',
      search: 'ابحث عن منتج...',
      searchLabel: 'بحث',
      wishlist: 'المفضلة',
      cart: 'السلة',
      menu: 'القائمة',
      closeMenu: 'إغلاق القائمة',
      theme: 'تبديل المظهر',
      language: 'English',
    },
    breadcrumb: { home: 'الرئيسية' },
    cart: {
      title: 'سلة التسوق',
      empty: 'سلتك فارغة',
      emptyDescription: 'تصفّح المتجر وأضف ما يعجبك، وستجده هنا.',
      continueShopping: 'متابعة التسوق',
      cancel: 'إلغاء',
      itemCount: '{count} منتج',
      product: 'المنتج',
      unitPrice: 'سعر الوحدة',
      quantity: 'الكمية',
      lineTotal: 'الإجمالي',
      remove: 'إزالة',
      removeLabel: 'إزالة {name} من السلة',
      clear: 'إفراغ السلة',
      clearConfirmTitle: 'إفراغ السلة؟',
      clearConfirmBody: 'ستُزال كل المنتجات من سلتك. لا يمكن التراجع.',
      summary: 'ملخص الطلب',
      subtotal: 'المجموع الفرعي',
      discount: 'الخصم',
      total: 'الإجمالي',
      checkout: 'إتمام الشراء',
      promoTitle: 'كود الخصم',
      promoPlaceholder: 'أدخل الكود',
      promoApply: 'تطبيق',
      promoApplying: 'جارٍ التحقق…',
      promoRemove: 'إزالة الكود',
      promoApplied: 'تم تطبيق الكود {code}',
      addedToCart: 'أُضيف إلى السلة',
      updating: 'جارٍ التحديث…',
      increase: 'زيادة الكمية',
      decrease: 'إنقاص الكمية',
      quantityLabel: 'كمية {name}',
      priceChangedNotice: 'الأسعار المعروضة هي الأسعار الحالية، وقد تختلف عمّا رأيته سابقًا.',
      lineOutOfStock: 'لم يعد متوفرًا — أزِله لإتمام الطلب.',
      lineQuantityReduced: 'المتوفر {count} فقط، وقد عُدّلت الكمية.',
      removedTitle: 'أُزيلت منتجات لم تعد متاحة',
      removedLine: '{name} ({sku})',
      errors: {
        outOfStock: 'هذا المنتج غير متوفر حاليًا.',
        onlySoManyLeft: 'المتوفر {count} فقط من هذا المنتج.',
        invalidQuantity: 'الكمية غير صحيحة.',
      },
      couponErrors: {
        invalid: 'هذا الكود غير صالح.',
        expired: 'انتهت صلاحية هذا الكود.',
        notStarted: 'هذا الكود لم يبدأ بعد.',
        noLongerAvailable: 'لم يعد هذا الكود متاحًا.',
        alreadyUsed: 'لقد استخدمت هذا الكود من قبل.',
        belowMinimum: 'هذا الكود يتطلب حدًّا أدنى للطلب قدره {amount}.',
        belowMinimumGeneric: 'قيمة سلتك أقل من الحد الأدنى المطلوب لهذا الكود.',
        notForTheseItems: 'هذا الكود لا ينطبق على المنتجات الموجودة في سلتك.',
      },
    },
    listing: {
      filters: 'الفلاتر',
      filtersOpen: 'عرض الفلاتر',
      sort: 'الترتيب',
      sortNewest: 'الأحدث',
      sortPriceAsc: 'السعر: من الأقل للأعلى',
      sortPriceDesc: 'السعر: من الأعلى للأقل',
      sortFeatured: 'مميز',
      clearFilters: 'مسح الفلاتر',
      brand: 'العلامة التجارية',
      price: 'السعر',
      priceMin: 'الحد الأدنى',
      priceMax: 'الحد الأقصى',
      availability: 'التوفر',
      inStockOnly: 'المتوفر فقط',
      apply: 'تطبيق',
      resultsCount: (count: number) => `${count} نتيجة`,
      noResultsTitle: 'لا توجد نتائج',
      noResultsDescription: 'جرّب تعديل الفلاتر أو كلمات بحث مختلفة.',
      categoryNotFoundTitle: 'الفئة غير موجودة',
      categoryNotFoundDescription: 'ربما تم نقلها أو حذفها.',
    },
    /** The wishlist page (`/[locale]/wishlist`). The header has linked here
     * since P05; the page itself landed later. */
    wishlistPage: {
      title: 'المفضلة',
      description: 'المنتجات اللي حفظتها للرجوع إليها لاحقًا.',
      /** A template, not a function: this label crosses the Server →
       * Client boundary (the count is only known in the browser), and a
       * function cannot. Same `{count}` convention the admin
       * dictionary uses. */
      countLabel: '{count} منتج',
      emptyTitle: 'قائمة المفضلة فاضية',
      emptyDescription: 'اضغط على القلب في أي منتج عشان تحفظه هنا وترجع له بعدين.',
      browse: 'تصفّح المنتجات',
      loading: 'جارٍ التحميل…',
      storageNotice:
        'المفضلة محفوظة في هذا المتصفح فقط — مش هتلاقيها على جهاز تاني، ولو مسحت بيانات المتصفح هتروح.',
      unavailableNotice: 'بعض المنتجات اللي حفظتها لم تعد متاحة، وتم استبعادها من القائمة.',
    },
    product: {
      addToCart: 'أضف إلى السلة',
      buyNow: 'اشترِ الآن',
      wishlistAdd: 'أضف إلى المفضلة',
      wishlistRemove: 'إزالة من المفضلة',
      quantity: 'الكمية',
      sku: 'رمز المنتج',
      specifications: 'المواصفات',
      description: 'الوصف',
      reviews: 'التقييمات',
      relatedProducts: 'منتجات ذات صلة',
      recentlyViewed: 'شوهد مؤخرًا',
      noReviewsTitle: 'لا توجد تقييمات بعد',
      noReviewsDescription: 'كن أول من يقيّم هذا المنتج.',
      notFoundTitle: 'المنتج غير موجود',
      notFoundDescription: 'ربما تم إلغاء نشره أو حذفه.',
      shippingInfo: 'يُشحن خلال 3–5 أيام عمل',
    },
    search: {
      title: (q: string) => `نتائج البحث عن «${q}»`,
      emptyTitle: 'ابحث في المتجر',
      emptyDescription: 'اكتب كلمة للبحث عن منتج، علامة تجارية، أو فئة.',
    },
    common: {
      currency: 'ر.س',
      loading: 'جارٍ التحميل...',
      errorTitle: 'حدث خطأ غير متوقع',
      errorDescription: 'حاول مرة أخرى، وإن تكرر تواصل معنا.',
      retry: 'إعادة المحاولة',
      imageUnavailable: 'الصورة غير متاحة',
    },
    payment: {
      title: 'الدفع',
      payNow: 'ادفع الآن',
      opening: 'جارٍ فتح صفحة الدفع…',
      redirectNotice: 'سنحوّلك إلى صفحة الدفع الآمنة لدى مزوّد الدفع.',
      securedBy: 'الدفع يتم لدى مزوّد الدفع — لا نحفظ بيانات بطاقتك ولا نراها.',
      statusLabel: 'حالة الدفع',
      amountLabel: 'المبلغ المستحق',
      referenceLabel: 'مرجع العملية',
      attemptsTitle: 'محاولات الدفع',
      attemptDate: 'التاريخ',
      attemptStatus: 'الحالة',
      attemptAmount: 'المبلغ',
      noAttempts: 'لم تبدأ أي محاولة دفع بعد.',
      checkAgain: 'تحديث حالة الدفع',
      checking: 'جارٍ التحقق…',
      retry: 'إعادة محاولة الدفع',
      // The return page: nothing here is a claim about the outcome.
      returnTitle: 'عدنا من صفحة الدفع',
      returnPendingTitle: 'ما زلنا نتحقق من عملية الدفع',
      returnPendingBody:
        'نتحقق من العملية مع مزوّد الدفع. قد يستغرق ذلك لحظات — هذه الصفحة لا تؤكّد الدفع بنفسها.',
      returnPaidTitle: 'تم تأكيد الدفع',
      returnPaidBody: 'استلمنا تأكيد الدفع من مزوّد الدفع.',
      returnFailedTitle: 'لم تكتمل عملية الدفع',
      returnFailedBody: 'لم يكتمل الدفع. طلبك محفوظ ويمكنك المحاولة مرة أخرى.',
      pendingBadge: 'بانتظار تأكيد الدفع',
      unavailableTitle: 'الدفع الإلكتروني غير مفعّل',
      unavailableBody: 'سنتواصل معك لترتيب الدفع.',
      errorNotFound: 'تعذّر العثور على الطلب.',
      errorAlreadyPaid: 'هذا الطلب مدفوع بالفعل.',
      errorNotPayable: 'لا يمكن دفع هذا الطلب في حالته الحالية.',
      errorUnavailable: 'الدفع الإلكتروني غير متاح حاليًا.',
      errorProvider: 'تعذّر فتح صفحة الدفع. حاول مرة أخرى.',
      errorGeneric: 'تعذّر إتمام العملية. حاول مرة أخرى.',
      statusCreated: 'بانتظار الدفع',
      statusRequiresAction: 'بحاجة إلى تأكيد إضافي',
      statusPending: 'قيد المعالجة',
      statusSucceeded: 'ناجحة',
      statusFailed: 'فاشلة',
      statusCancelled: 'ملغاة',
      statusExpired: 'منتهية الصلاحية',
    },
    checkout: {
      title: 'إتمام الطلب',
      contactSection: 'بيانات التواصل',
      addressSection: 'عنوان الشحن',
      noteSection: 'ملاحظات الطلب',
      summarySection: 'ملخص الطلب',
      paymentSection: 'الدفع',
      email: 'البريد الإلكتروني',
      phone: 'رقم الجوال',
      phoneHint: 'مثال: 0512345678',
      fullName: 'الاسم الكامل',
      city: 'المدينة',
      district: 'الحي',
      street: 'الشارع',
      buildingNumber: 'رقم المبنى',
      additionalNumber: 'الرقم الإضافي',
      postalCode: 'الرمز البريدي',
      optional: 'اختياري',
      addressNotes: 'تفاصيل إضافية للتوصيل',
      orderNote: 'ملاحظة على الطلب',
      placeOrder: 'تأكيد الطلب',
      placing: 'جارٍ تأكيد الطلب…',
      paymentPendingTitle: 'الدفع غير متاح بعد',
      paymentPendingBody:
        'سيُسجَّل طلبك الآن بحالة «بانتظار الدفع»، وسنتواصل معك لإتمام الدفع. الدفع الإلكتروني قادم في مرحلة لاحقة.',
      emptyTitle: 'سلتك فارغة',
      emptyBody: 'أضف منتجًا إلى السلة قبل إتمام الطلب.',
      backToCart: 'العودة إلى السلة',
      browseProducts: 'تصفّح المنتجات',
      required: 'هذا الحقل مطلوب',
      invalidEmail: 'أدخل بريدًا إلكترونيًا صحيحًا',
      invalidPhone: 'أدخل رقم جوال سعودي صحيح',
      invalidPostalCode: 'الرمز البريدي يتكوّن من 5 أرقام',
      invalidAdditionalNumber: 'الرقم الإضافي يتكوّن من 4 أرقام',
      fixErrors: 'راجع الحقول المميّزة أدناه',
      errorCartEmpty: 'سلتك فارغة. أضف منتجًا ثم أعد المحاولة.',
      errorOutOfStock: 'لم تعد الكمية المطلوبة متوفّرة. راجع سلتك وأعد المحاولة.',
      errorCouponInvalid: 'كود الخصم لم يعد صالحًا. أزِله أو استبدله ثم أعد المحاولة.',
      errorPriceChanged: 'تغيّر سعر أحد المنتجات. راجع الإجمالي الجديد قبل التأكيد.',
      errorGeneric: 'تعذّر إتمام الطلب. حاول مرة أخرى.',
      subtotal: 'المجموع الفرعي',
      discount: 'الخصم',
      shipping: 'الشحن',
      tax: 'الضريبة',
      total: 'الإجمالي',
      notCalculated: 'يُحدَّد لاحقًا',
      itemsTitle: 'المنتجات',
      quantityLabel: 'الكمية',
    },
    orders: {
      successTitle: 'تم استلام طلبك',
      successBody: 'حفظنا طلبك وسنتواصل معك على بيانات التواصل أدناه.',
      nextStepsTitle: 'الخطوة التالية',
      nextStepsBody: 'سنتواصل معك لتأكيد الطلب وترتيب الدفع والتوصيل.',
      saveLinkNotice: 'احتفظ بهذا الرابط لمتابعة طلبك لاحقًا.',
      orderNumber: 'رقم الطلب',
      placedAt: 'تاريخ الطلب',
      viewOrder: 'عرض الطلب',
      myOrders: 'طلباتي',
      noOrders: 'لا توجد طلبات بعد',
      noOrdersBody: 'عندما تُتم أول طلب سيظهر هنا.',
      date: 'التاريخ',
      total: 'الإجمالي',
      items: 'المنتجات',
      itemsCount: '{count} منتج',
      view: 'عرض',
      backToOrders: 'العودة إلى الطلبات',
      contactDetails: 'بيانات التواصل',
      shippingAddress: 'عنوان الشحن',
      itemsTitle: 'المنتجات',
      timeline: 'سجل الطلب',
      orderNote: 'ملاحظة الطلب',
      notFound: 'الطلب غير موجود',
      notFoundBody: 'تحقّق من الرابط أو رقم الطلب.',
      cancel: 'إلغاء الطلب',
      cancelConfirmTitle: 'إلغاء الطلب؟',
      cancelConfirmBody: 'سيُلغى الطلب وتُعاد الكميات إلى المخزون. لا يمكن التراجع.',
      cancelConfirm: 'تأكيد الإلغاء',
      cancelled: 'تم إلغاء الطلب',
      cancelFailed: 'تعذّر إلغاء الطلب',
      statusLabel: 'حالة الطلب',
      paymentLabel: 'حالة الدفع',
      fulfillmentLabel: 'حالة التنفيذ',
      statusPendingPayment: 'بانتظار الدفع',
      statusPendingManual: 'بانتظار التأكيد',
      statusConfirmed: 'مؤكَّد',
      statusProcessing: 'قيد التجهيز',
      statusCompleted: 'مكتمل',
      statusCancelled: 'ملغى',
      paymentUnpaid: 'غير مدفوع',
      paymentPending: 'قيد المعالجة',
      paymentPaid: 'مدفوع',
      paymentFailed: 'فشل الدفع',
      paymentRefunded: 'مسترد',
      paymentPartiallyRefunded: 'مسترد جزئيًا',
      fulfillmentUnfulfilled: 'لم يُشحن',
      fulfillmentProcessing: 'قيد التجهيز',
      fulfillmentShipped: 'تم الشحن',
      fulfillmentDelivered: 'تم التسليم',
      fulfillmentCancelled: 'ملغى',
      eventCreated: 'أُنشئ الطلب',
      eventOrderStatus: 'تغيّرت حالة الطلب',
      eventPaymentStatus: 'تغيّرت حالة الدفع',
      eventFulfillmentStatus: 'تغيّرت حالة التنفيذ',
      eventNote: 'ملاحظة',
      systemActor: 'النظام',
    },
    account: {
      // Shared auth chrome
      emailLabel: 'البريد الإلكتروني',
      emailPlaceholder: 'name@example.com',
      passwordLabel: 'كلمة المرور',
      passwordPlaceholder: '••••••••',
      passwordConfirmationLabel: 'تأكيد كلمة المرور',
      nameLabel: 'الاسم الكامل',
      phoneLabel: 'رقم الجوال',
      optional: 'اختياري',
      showPassword: 'إظهار كلمة المرور',
      hidePassword: 'إخفاء كلمة المرور',

      // Login
      loginTitle: 'تسجيل الدخول',
      loginSubtitle: 'سجّل الدخول لمتابعة طلباتك وحسابك.',
      signIn: 'تسجيل الدخول',
      signingIn: 'جارٍ تسجيل الدخول…',
      noAccountYet: 'ليس لديك حساب؟',
      registerLink: 'إنشاء حساب',
      forgotPasswordLink: 'نسيت كلمة المرور؟',
      errorValidation: 'تحقّق من البريد الإلكتروني وكلمة المرور.',
      errorRateLimited: 'محاولات كثيرة. انتظر قليلًا ثم أعد المحاولة.',
      errorInvalidCredentials: 'البريد الإلكتروني أو كلمة المرور غير صحيحة.',

      // Register
      registerTitle: 'إنشاء حساب',
      registerSubtitle: 'أنشئ حسابًا لمتابعة طلباتك بسهولة في كل مرة.',
      createAccount: 'إنشاء الحساب',
      creatingAccount: 'جارٍ إنشاء الحساب…',
      alreadyHaveAccount: 'لديك حساب بالفعل؟',
      signInLink: 'تسجيل الدخول',
      errorEmailTaken: 'هذا البريد الإلكتروني مسجَّل بالفعل. جرّب تسجيل الدخول.',
      errorPasswordMismatch: 'كلمتا المرور غير متطابقتين.',
      errorGeneric: 'تعذّر إتمام الطلب. حاول مرة أخرى.',
      passwordHint: '٨ أحرف على الأقل، تتضمّن حرفًا ورقمًا.',

      // Forgot / reset password
      forgotPasswordTitle: 'نسيت كلمة المرور',
      forgotPasswordSubtitle: 'أدخل بريدك الإلكتروني وسنرسل رابطًا لإعادة تعيين كلمة المرور.',
      forgotPasswordSubmit: 'إرسال رابط إعادة التعيين',
      forgotPasswordSubmitting: 'جارٍ الإرسال…',
      forgotPasswordSuccessTitle: 'تحقّق من بريدك الإلكتروني',
      forgotPasswordSuccessBody:
        'إذا كان هناك حساب مرتبط بهذا البريد، فسيصلك رابط لإعادة تعيين كلمة المرور.',
      backToLogin: 'العودة إلى تسجيل الدخول',
      resetPasswordTitle: 'إعادة تعيين كلمة المرور',
      resetPasswordSubtitle: 'اختر كلمة مرور جديدة لحسابك.',
      resetPasswordSubmit: 'حفظ كلمة المرور الجديدة',
      resetPasswordSubmitting: 'جارٍ الحفظ…',
      resetPasswordSuccessTitle: 'تم تغيير كلمة المرور',
      resetPasswordSuccessBody: 'يمكنك الآن تسجيل الدخول بكلمة المرور الجديدة.',
      resetPasswordErrorInvalid: 'رابط إعادة التعيين غير صالح.',
      resetPasswordErrorExpired: 'انتهت صلاحية رابط إعادة التعيين. اطلب رابطًا جديدًا.',
      resetPasswordErrorUsed: 'تم استخدام هذا الرابط بالفعل. اطلب رابطًا جديدًا.',
      goToLogin: 'الذهاب إلى تسجيل الدخول',

      // Email verification
      verifyEmailTitle: 'تأكيد البريد الإلكتروني',
      verifyEmailSuccessTitle: 'تم تأكيد بريدك الإلكتروني',
      verifyEmailSuccessBody: 'شكرًا لتأكيد بريدك الإلكتروني.',
      verifyEmailErrorInvalid: 'رابط التأكيد غير صالح.',
      verifyEmailErrorExpired: 'انتهت صلاحية رابط التأكيد. اطلب رابطًا جديدًا من صفحة حسابك.',
      verifyEmailErrorUsed: 'تم استخدام رابط التأكيد هذا بالفعل.',
      verifyEmailPendingNotice: 'بريدك الإلكتروني غير مؤكَّد بعد.',
      resendVerification: 'إعادة إرسال رابط التأكيد',
      resendingVerification: 'جارٍ الإرسال…',
      resendVerificationSuccess: 'أُرسل رابط تأكيد جديد.',

      // Account shell
      accountTitle: 'حسابي',
      navOverview: 'نظرة عامة',
      navProfile: 'الملف الشخصي',
      navOrders: 'طلباتي',
      navBodyProfile: 'ملف المقاسات',
      navSignOut: 'تسجيل الخروج',
      signingOut: 'جارٍ تسجيل الخروج…',
      welcomeBack: 'مرحبًا، {name}',
      overviewOrdersTitle: 'طلباتك',
      overviewOrdersBody: 'راجع طلباتك السابقة وتتبّع حالتها.',
      overviewOrdersCta: 'عرض الطلبات',
      overviewProfileTitle: 'ملفك الشخصي',
      overviewProfileBody: 'حدّث اسمك ورقم جوالك.',
      overviewProfileCta: 'تعديل الملف الشخصي',

      // Profile
      profileFormTitle: 'المعلومات الشخصية',
      emailReadOnlyNote: 'لا يمكن تغيير البريد الإلكتروني حاليًا من هنا.',
      saveChanges: 'حفظ التغييرات',
      saving: 'جارٍ الحفظ…',
      profileUpdated: 'تم تحديث ملفك الشخصي',
      profileValidationError: 'تحقّق من البيانات المُدخلة.',
      profileError: 'تعذّر حفظ التغييرات. حاول مرة أخرى.',
      sessionExpired: 'انتهت جلستك. سجّل الدخول مرة أخرى.',
      unauthorized: 'يلزم تسجيل الدخول لعرض هذه الصفحة.',
    },

    // Clothing P01 — the customer's body profile and avatar. A clothing
    // personalisation tool, worded as one: plain language, no clinical
    // terms, and every appearance option phrased as the customer's choice.
    bodyProfile: {
      title: 'ملف المقاسات',
      intro:
        'أضف مقاساتك مرة واحدة لنساعدك في اختيار المقاس الأنسب. هذه البيانات لك وحدك، ويمكنك تعديلها أو حذفها في أي وقت.',
      privacyNote: 'لا نطلب صورتك ولا نخمّن شيئًا عن مظهرك — كل ما يظهر هنا هو ما تختاره أنت.',
      requiredMark: 'مطلوب',
      optionalMark: 'اختياري',
      units: { cm: 'سم', kg: 'كجم' },
      sections: {
        basic: { title: 'المعلومات الأساسية', description: 'لنعرض لك القَصّات المناسبة أولًا.' },
        measurements: {
          title: 'المقاسات',
          description:
            'استخدم شريط قياس مرن وقِف بشكل طبيعي. الحقول المطلوبة تكفي للبدء، والباقي يجعل الاقتراحات أدق.',
          required: 'الأساسية',
          optional: 'لدقة أعلى',
        },
        appearance: {
          title: 'المظهر',
          description: 'اختياري بالكامل — يغيّر شكل الصورة التوضيحية فقط، ولا يؤثر على المقاسات.',
        },
      },
      fitPreference: {
        label: 'القَصّة التي تفضّلها',
        help: 'تحدد المساحة التي نتركها عند اقتراح المقاس — ولا تغيّر مقاساتك.',
        options: { SLIM: 'ضيقة', REGULAR: 'عادية', RELAXED: 'مريحة' },
      },
      gender: {
        label: 'الجنس',
        help: 'يساعدنا في عرض القَصّات المناسبة لك أولًا. اختيارك أنت — لا نستنتجه من أي شيء.',
        options: {
          FEMALE: 'أنثى',
          MALE: 'ذكر',
          OTHER: 'آخر',
          PREFER_NOT_TO_SAY: 'أفضّل عدم الإجابة',
        },
      },
      measurements: {
        heightCm: { label: 'الطول', help: 'قِف مستقيمًا بجانب الحائط وبدون حذاء.' },
        weightKg: {
          label: 'الوزن',
          help: 'وزنك المعتاد. نستخدمه كمعلومة مساعدة فقط، ولا نختار المقاس على أساسه وحده.',
        },
        waistCm: {
          label: 'محيط الخصر',
          help: 'قِس حول خصرك الطبيعي (أضيق جزء) دون شدّ الشريط.',
        },
        chestCm: {
          label: 'محيط الصدر',
          help: 'حول أعرض جزء من الصدر تحت الإبطين، والشريط مستوٍ.',
        },
        hipCm: { label: 'محيط الورك', help: 'حول أعرض جزء من الوركين، والقدمان متلاصقتان.' },
        shoulderCm: { label: 'عرض الكتفين', help: 'من طرف الكتف إلى الطرف الآخر عبر الظهر.' },
        inseamCm: { label: 'طول الساق الداخلي', help: 'من أعلى الفخذ من الداخل حتى الكاحل.' },
        sleeveLengthCm: {
          label: 'طول الكم',
          help: 'من طرف الكتف حتى الرسغ، والذراع مسترخية.',
        },
        neckCm: { label: 'محيط الرقبة', help: 'حول قاعدة الرقبة مع ترك مسافة إصبع تحت الشريط.' },
      },
      guide: {
        open: 'كيف أقيس؟',
        title: 'كيف تأخذ مقاساتك',
        intro: 'دقيقتان وشريط قياس مرن هما كل ما تحتاجه. هذه نصائح عامة تنطبق على كل المقاسات:',
        tips: [
          'قِس فوق ملابس خفيفة، أو بدونها إن أمكن.',
          'أبقِ الشريط مستويًا وملاصقًا للجسم دون أن يضغط عليه.',
          'قِف بشكل طبيعي وتنفّس بهدوء — لا تشدّ بطنك.',
          'إذا كنت بين رقمين فاختر الأكبر، ويمكنك تعديله لاحقًا.',
        ],
        close: 'فهمت',
      },
      appearance: {
        notSpecified: 'بدون تحديد',
        skinTone: {
          label: 'لون البشرة',
          options: {
            LIGHT: 'فاتحة',
            MEDIUM_LIGHT: 'حنطية فاتحة',
            MEDIUM: 'حنطية',
            MEDIUM_DARK: 'سمراء',
            DARK: 'داكنة',
          },
        },
        hairStyle: {
          label: 'تسريحة الشعر',
          options: {
            SHORT: 'قصير',
            MEDIUM: 'متوسط',
            LONG: 'طويل',
            STRAIGHT: 'ناعم',
            WAVY: 'مموّج',
            CURLY: 'مجعّد',
            BUZZ: 'قصير جدًا',
            COVERED: 'حجاب / غطاء رأس',
          },
        },
        hairColor: {
          label: 'لون الشعر',
          coveredNote: 'لا حاجة لاختيار لون الشعر مع غطاء الرأس.',
          options: {
            BLACK: 'أسود',
            DARK_BROWN: 'بني داكن',
            BROWN: 'بني',
            LIGHT_BROWN: 'بني فاتح',
            BLONDE: 'أشقر',
            RED: 'أحمر',
            GRAY: 'رمادي',
            WHITE: 'أبيض',
          },
        },
        facialHair: {
          label: 'شعر الوجه',
          options: {
            NONE: 'بدون',
            SHORT_BEARD: 'لحية قصيرة',
            LONG_BEARD: 'لحية طويلة',
            MUSTACHE: 'شارب',
          },
        },
        glasses: {
          label: 'أرتدي نظارة',
          style: {
            label: 'شكل النظارة',
            options: {
              ROUND: 'دائرية',
              RECTANGULAR: 'مستطيلة',
              AVIATOR: 'أفياتور',
              CAT_EYE: 'عين القطة',
            },
          },
          frameColor: {
            label: 'لون الإطار',
            options: {
              BLACK: 'أسود',
              BROWN: 'بني',
              GOLD: 'ذهبي',
              SILVER: 'فضي',
              CLEAR: 'شفاف',
            },
          },
        },
      },
      preview: {
        title: 'صورتك التوضيحية',
        caption: 'رسم توضيحي من مقاساتك واختياراتك — وليس صورة حقيقية.',
        heightUnset: 'أدخل طولك لرسم الصورة بالحجم الصحيح.',
        glasses: 'نظارة',
        bodyShape: {
          STRAIGHT: 'قوام متناسق الخطوط',
          DEFINED_WAIST: 'خصر واضح',
          HIPS_WIDER: 'الورك أعرض من الصدر',
          CHEST_WIDER: 'الصدر أعرض من الورك',
          WAIST_WIDEST: 'الخصر أعرض من الصدر والورك',
        },
      },
      completion: {
        title: 'اكتمال الملف',
        percent: 'ملفك مكتمل بنسبة {percent}%',
        complete: 'ملفك مكتمل. أحسنت!',
        sectionDone: 'مكتمل',
        sectionProgress: '{done} من {total}',
        requiredMissing: 'أكمل الحقول المطلوبة لتتمكن من الحفظ.',
        next: {
          gender: 'اختر الجنس لنعرض لك القَصّات المناسبة.',
          heightCm: 'أضف طولك — مطلوب للحفظ.',
          weightKg: 'أضف وزنك — مطلوب للحفظ.',
          waistCm: 'أضف محيط الخصر — مطلوب للحفظ.',
          hipCm: 'أضف محيط الورك لتحسين اقتراحات المقاس.',
          chestCm: 'أضف محيط الصدر لقمصان وبلوزات بمقاس أدق.',
          inseamCm: 'أضف طول الساق الداخلي لبناطيل بطول مناسب.',
          shoulderCm: 'أضف عرض الكتفين للجاكيتات والقمصان.',
          sleeveLengthCm: 'أضف طول الكم للأكمام الطويلة.',
          neckCm: 'أضف محيط الرقبة لياقات القمصان.',
          skinTone: 'اختر لون البشرة لصورتك التوضيحية.',
          hairStyle: 'اختر تسريحة الشعر لصورتك التوضيحية.',
          hairColor: 'اختر لون الشعر لصورتك التوضيحية.',
        },
      },
      errors: {
        required: 'هذا الحقل مطلوب.',
        invalid_number: 'أدخل رقمًا فقط، مثل 88 أو 88.5.',
        too_small: 'القيمة أقل من المتوقع — أدخل {min} {unit} على الأقل.',
        too_large: 'القيمة أكبر من المتوقع — أدخل {max} {unit} على الأكثر.',
        invalid_option: 'اختر أحد الخيارات المتاحة.',
        summary: 'راجع الحقول المعلَّمة ثم احفظ مرة أخرى.',
        stale: 'تم تعديل ملفك من نافذة أخرى. حدّث الصفحة ثم أعد المحاولة.',
        sessionExpired: 'انتهت جلستك. سجّل الدخول مرة أخرى.',
        generic: 'تعذّر الحفظ الآن. حاول مرة أخرى.',
      },
      save: 'حفظ الملف',
      saving: 'جارٍ الحفظ…',
      saved: 'تم حفظ ملف المقاسات',
      unsaved: 'تغييرات غير محفوظة',
      delete: {
        button: 'حذف ملف المقاسات',
        title: 'حذف ملف المقاسات؟',
        description: 'سيتم حذف مقاساتك وإعدادات المظهر نهائيًا. يمكنك إنشاء ملف جديد في أي وقت.',
        confirm: 'حذف نهائيًا',
        cancel: 'إلغاء',
        deleting: 'جارٍ الحذف…',
        deleted: 'تم حذف ملف المقاسات',
      },
    },

    // Clothing P02 — the size recommendation on product pages. Every line is
    // a sentence about the garment and the customer's own choices; no
    // measurement value ever appears in this copy.
    sizing: {
      title: 'المقاس والقَصّة',
      chartRecommended: 'المقترح لك',
      recommended: 'مقاسك المقترح: {size}',
      loading: 'نحسب مقاسك…',
      confidence: { HIGH: 'ثقة عالية', MEDIUM: 'ثقة متوسطة', LOW: 'ثقة منخفضة' },
      confidenceHelp: {
        HIGH: 'مقاساتك تطابق هذا المقاس بوضوح.',
        MEDIUM: 'مناسب لك، مع فرق بسيط في بعض القياسات.',
        LOW: 'أقرب مقاس متاح لك، لكنه ليس مطابقًا تمامًا.',
      },
      why: 'لماذا هذا المقاس؟',
      fitLabel: 'القَصّة',
      fitOptions: { SLIM: 'ضيقة', REGULAR: 'عادية', RELAXED: 'مريحة' },
      fits: {
        fits: 'مناسب',
        snug: 'ضيق قليلًا',
        loose: 'واسع قليلًا',
        too_tight: 'ضيق',
        too_loose: 'واسع',
      },
      reasonMeasurement: 'قياس {measurement}: {fit}',
      reasonFit: {
        SLIM: 'حسب تفضيلك للقَصّة الضيقة',
        REGULAR: 'حسب تفضيلك للقَصّة العادية',
        RELAXED: 'حسب تفضيلك للقَصّة المريحة',
      },
      reasonLengthFromHeight: 'الطول محسوب تقديريًا من طولك',
      reasonBetweenSizes: 'أنت بين مقاسين — {size} خيار قريب أيضًا',
      reasonLimited: 'أضف {measurements} لنتيجة أدق',
      measurementNames: {
        chestCm: 'الصدر',
        waistCm: 'الخصر',
        hipCm: 'الورك',
        shoulderCm: 'الأكتاف',
        sleeveCm: 'الكم',
        inseamCm: 'طول الساق',
        lengthCm: 'الطول',
        neckCm: 'الرقبة',
      },
      profileMeasurementNames: {
        heightCm: 'طولك',
        waistCm: 'محيط الخصر',
        chestCm: 'محيط الصدر',
        hipCm: 'محيط الورك',
        shoulderCm: 'عرض الكتفين',
        inseamCm: 'طول الساق الداخلي',
        sleeveLengthCm: 'طول الكم',
        neckCm: 'محيط الرقبة',
      },
      listJoin: ' و',
      alternatives: 'بدائل',
      alternative: { smaller: '{size} — أضيق قليلًا', larger: '{size} — أوسع قليلًا' },
      selectSize: 'اختر المقاس {size}',
      selectedSize: 'المقاس {size} مختار',
      insufficient: 'أضف {measurements} إلى ملف مقاساتك لنقترح مقاس {item}.',
      items: {
        T_SHIRT: 'هذا التيشيرت',
        SHIRT: 'هذا القميص',
        HOODIE: 'هذا الهودي',
        SWEATER: 'هذه الكنزة',
        JACKET: 'هذا الجاكيت',
        CARDIGAN: 'هذا الكارديغان',
        DRESS: 'هذا الفستان',
        ABAYA: 'هذه العباية',
        THOBE: 'هذا الثوب',
        JEANS: 'هذا الجينز',
        TROUSERS: 'هذا البنطلون',
        SHORTS: 'هذا الشورت',
        SKIRT: 'هذه التنورة',
      },
      updateProfile: 'حدّث ملف مقاساتك',
      noMatch: {
        above: 'مقاساتك أكبر من أكبر مقاس متوفر في هذا المنتج.',
        below: 'مقاساتك أصغر من أصغر مقاس متوفر في هذا المنتج.',
        mixed: 'لا يوجد مقاس في هذا المنتج يناسب مقاساتك بشكل جيد.',
      },
      noMatchHelp: 'راجع جدول المقاسات، أو جرّب قَصّة أخرى.',
      signedOut: {
        title: 'اعرف مقاسك قبل الشراء',
        body: 'أنشئ ملف مقاساتك مرة واحدة، ونقترح عليك المقاس الأنسب لكل قطعة.',
        cta: 'أنشئ ملف مقاساتك',
      },
      noProfile: {
        title: 'اعرف مقاسك قبل الشراء',
        body: 'أضف مقاساتك مرة واحدة، ونقترح عليك المقاس الأنسب لكل قطعة.',
        cta: 'أنشئ ملف مقاساتك',
      },
      error: 'تعذّر حساب المقاس الآن. يمكنك الاستعانة بجدول المقاسات.',
      howItWorks: 'كيف نحسبه؟',
      howItWorksBody:
        'نقارن قياسات كل مقاس بمقاساتك، مع المساحة المناسبة للقَصّة التي تفضّلها. درجة الثقة تعكس مدى اكتمال القياسات وتطابقها — وهي قاعدة واضحة وليست نسبة إحصائية.',
      privacy: 'نحسب المقاس من ملفك على خوادمنا، ولا نعرض مقاساتك هنا.',
      sizeGuide: {
        open: 'جدول المقاسات',
        title: 'جدول المقاسات',
        description: 'قياسات القطعة نفسها وهي مفرودة، بالسنتيمتر.',
        size: 'المقاس',
        unit: 'سم',
      },
    },

    // P13 — transactional email copy. Deliberately its own section, not
    // reused from `account`: an email template has no session, no button
    // states, no live validation — it is static copy read once, so it gets
    // its own small, complete set of strings rather than borrowing UI
    // strings that assume a page around them.
    email: {
      greeting: 'مرحبًا {name}،',
      greetingNoName: 'مرحبًا،',
      fallbackNotice: 'إذا لم يعمل الزر، انسخ الرابط التالي والصقه في متصفحك:',
      footer: '© {year} {store}. جميع الحقوق محفوظة.',
      automatedNotice: 'هذه رسالة آلية، يرجى عدم الرد عليها.',

      verificationSubject: 'تأكيد بريدك الإلكتروني — {store}',
      verificationHeading: 'تأكيد بريدك الإلكتروني',
      verificationBody:
        'شكرًا لإنشاء حساب في {store}. لإتمام إعداد حسابك، يرجى تأكيد أن هذا هو بريدك الإلكتروني.',
      verificationCta: 'تأكيد البريد الإلكتروني',
      verificationExpiry: 'ينتهي هذا الرابط خلال 24 ساعة.',
      verificationIgnore: 'إذا لم تُنشئ هذا الحساب، يمكنك تجاهل هذه الرسالة بأمان.',

      passwordResetSubject: 'إعادة تعيين كلمة المرور — {store}',
      passwordResetHeading: 'إعادة تعيين كلمة المرور',
      passwordResetBody: 'وصلنا طلب لإعادة تعيين كلمة مرور حسابك في {store}.',
      passwordResetCta: 'إعادة تعيين كلمة المرور',
      passwordResetExpiry: 'ينتهي هذا الرابط خلال ساعة واحدة.',
      passwordResetIgnore: 'إذا لم تطلب ذلك، يمكنك تجاهل هذه الرسالة بأمان — لن تتغيّر كلمة مرورك.',
    },
  },
  en: {
    /** The storefront footer. */
    footerSection: {
      contact: 'Contact us',
      whatsapp: 'WhatsApp',
      email: 'Email',
      phone: 'Phone',
      address: 'Address',
    },
    nav: {
      home: 'Home',
      allCategories: 'All Categories',
      search: 'Search products...',
      searchLabel: 'Search',
      wishlist: 'Wishlist',
      cart: 'Cart',
      menu: 'Menu',
      closeMenu: 'Close menu',
      theme: 'Toggle theme',
      language: 'العربية',
    },
    breadcrumb: { home: 'Home' },
    cart: {
      title: 'Shopping cart',
      empty: 'Your cart is empty',
      emptyDescription: 'Browse the store and add something you like — it will show up here.',
      continueShopping: 'Continue shopping',
      cancel: 'Cancel',
      itemCount: '{count} items',
      product: 'Product',
      unitPrice: 'Unit price',
      quantity: 'Quantity',
      lineTotal: 'Total',
      remove: 'Remove',
      removeLabel: 'Remove {name} from the cart',
      clear: 'Empty cart',
      clearConfirmTitle: 'Empty the cart?',
      clearConfirmBody: 'Every item will be removed from your cart. This cannot be undone.',
      summary: 'Order summary',
      subtotal: 'Subtotal',
      discount: 'Discount',
      total: 'Total',
      checkout: 'Checkout',
      promoTitle: 'Promotion code',
      promoPlaceholder: 'Enter a code',
      promoApply: 'Apply',
      promoApplying: 'Checking…',
      promoRemove: 'Remove code',
      promoApplied: 'Code {code} applied',
      addedToCart: 'Added to your cart',
      updating: 'Updating…',
      increase: 'Increase quantity',
      decrease: 'Decrease quantity',
      quantityLabel: 'Quantity of {name}',
      priceChangedNotice: 'Prices shown are current, and may differ from what you saw earlier.',
      lineOutOfStock: 'No longer available — remove it to check out.',
      lineQuantityReduced: 'Only {count} left, so the quantity was adjusted.',
      removedTitle: 'Items that are no longer available were removed',
      removedLine: '{name} ({sku})',
      errors: {
        outOfStock: 'This item is not available right now.',
        onlySoManyLeft: 'Only {count} of this item are available.',
        invalidQuantity: 'That quantity is not valid.',
      },
      couponErrors: {
        invalid: 'That code is not valid.',
        expired: 'That code has expired.',
        notStarted: 'That code is not active yet.',
        noLongerAvailable: 'That code is no longer available.',
        alreadyUsed: 'You have already used that code.',
        belowMinimum: 'That code needs a minimum order of {amount}.',
        belowMinimumGeneric: 'Your cart is below the minimum this code requires.',
        notForTheseItems: 'That code does not apply to the items in your cart.',
      },
    },
    listing: {
      filters: 'Filters',
      filtersOpen: 'Show filters',
      sort: 'Sort',
      sortNewest: 'Newest',
      sortPriceAsc: 'Price: Low to High',
      sortPriceDesc: 'Price: High to Low',
      sortFeatured: 'Featured',
      clearFilters: 'Clear filters',
      brand: 'Brand',
      price: 'Price',
      priceMin: 'Min',
      priceMax: 'Max',
      availability: 'Availability',
      inStockOnly: 'In stock only',
      apply: 'Apply',
      resultsCount: (count: number) => `${count} results`,
      noResultsTitle: 'No results found',
      noResultsDescription: 'Try adjusting your filters or search terms.',
      categoryNotFoundTitle: 'Category not found',
      categoryNotFoundDescription: 'It may have been moved or removed.',
    },
    /** The wishlist page (`/[locale]/wishlist`). The header has linked here
     * since P05; the page itself landed later. */
    wishlistPage: {
      title: 'Wishlist',
      description: 'Products you saved to come back to.',
      /** A template, not a function — see the Arabic entry. */
      countLabel: '{count} products',
      emptyTitle: 'Your wishlist is empty',
      emptyDescription: 'Tap the heart on any product to save it here for later.',
      browse: 'Browse products',
      loading: 'Loading…',
      storageNotice:
        'Your wishlist is saved in this browser only — it will not follow you to another device, and clearing your browser data clears it.',
      unavailableNotice: 'Some products you saved are no longer available and have been left out.',
    },
    product: {
      addToCart: 'Add to Cart',
      buyNow: 'Buy Now',
      wishlistAdd: 'Add to wishlist',
      wishlistRemove: 'Remove from wishlist',
      quantity: 'Quantity',
      sku: 'SKU',
      specifications: 'Specifications',
      description: 'Description',
      reviews: 'Reviews',
      relatedProducts: 'Related products',
      recentlyViewed: 'Recently viewed',
      noReviewsTitle: 'No reviews yet',
      noReviewsDescription: 'Be the first to review this product.',
      notFoundTitle: 'Product not found',
      notFoundDescription: 'It may have been unpublished or removed.',
      shippingInfo: 'Ships in 3–5 business days',
    },
    search: {
      title: (q: string) => `Search results for "${q}"`,
      emptyTitle: 'Search the store',
      emptyDescription: 'Type to search for a product, brand, or category.',
    },
    common: {
      currency: 'SAR',
      loading: 'Loading...',
      errorTitle: 'Something went wrong',
      errorDescription: 'Try again, and contact us if it keeps happening.',
      retry: 'Retry',
      imageUnavailable: 'Image unavailable',
    },
    payment: {
      title: 'Payment',
      payNow: 'Pay now',
      opening: 'Opening the payment page…',
      redirectNotice: 'We will take you to the payment provider’s secure page.',
      securedBy: 'Payment happens at the provider — we never see or store your card details.',
      statusLabel: 'Payment status',
      amountLabel: 'Amount due',
      referenceLabel: 'Transaction reference',
      attemptsTitle: 'Payment attempts',
      attemptDate: 'Date',
      attemptStatus: 'Status',
      attemptAmount: 'Amount',
      noAttempts: 'No payment has been started yet.',
      checkAgain: 'Check payment status',
      checking: 'Checking…',
      retry: 'Try payment again',
      returnTitle: 'Back from the payment page',
      returnPendingTitle: 'We are still confirming your payment',
      returnPendingBody:
        'We are checking with the payment provider. This can take a moment — this page does not confirm payment on its own.',
      returnPaidTitle: 'Payment confirmed',
      returnPaidBody: 'The provider confirmed your payment.',
      returnFailedTitle: 'Payment did not go through',
      returnFailedBody: 'The payment did not complete. Your order is saved and you can try again.',
      pendingBadge: 'Awaiting payment confirmation',
      unavailableTitle: 'Online payment is not switched on',
      unavailableBody: 'We will contact you to arrange payment.',
      errorNotFound: 'We could not find that order.',
      errorAlreadyPaid: 'This order is already paid.',
      errorNotPayable: 'This order cannot be paid in its current state.',
      errorUnavailable: 'Online payment is not available right now.',
      errorProvider: 'We could not open the payment page. Please try again.',
      errorGeneric: 'We could not complete that. Please try again.',
      statusCreated: 'Awaiting payment',
      statusRequiresAction: 'Needs extra confirmation',
      statusPending: 'Processing',
      statusSucceeded: 'Succeeded',
      statusFailed: 'Failed',
      statusCancelled: 'Cancelled',
      statusExpired: 'Expired',
    },
    checkout: {
      title: 'Checkout',
      contactSection: 'Contact details',
      addressSection: 'Shipping address',
      noteSection: 'Order notes',
      summarySection: 'Order summary',
      paymentSection: 'Payment',
      email: 'Email',
      phone: 'Mobile number',
      phoneHint: 'For example: 0512345678',
      fullName: 'Full name',
      city: 'City',
      district: 'District',
      street: 'Street',
      buildingNumber: 'Building number',
      additionalNumber: 'Additional number',
      postalCode: 'Postal code',
      optional: 'optional',
      addressNotes: 'Extra delivery details',
      orderNote: 'Order note',
      placeOrder: 'Place order',
      placing: 'Placing your order…',
      paymentPendingTitle: 'Payment is not available yet',
      paymentPendingBody:
        'Your order will be recorded as awaiting payment, and we will contact you to arrange it. Online payment arrives in a later phase.',
      emptyTitle: 'Your cart is empty',
      emptyBody: 'Add something to your cart before checking out.',
      backToCart: 'Back to cart',
      browseProducts: 'Browse products',
      required: 'This field is required',
      invalidEmail: 'Enter a valid email address',
      invalidPhone: 'Enter a valid Saudi mobile number',
      invalidPostalCode: 'A postal code is 5 digits',
      invalidAdditionalNumber: 'An additional number is 4 digits',
      fixErrors: 'Check the highlighted fields below',
      errorCartEmpty: 'Your cart is empty. Add something and try again.',
      errorOutOfStock: 'That quantity is no longer available. Review your cart and try again.',
      errorCouponInvalid: 'The promo code no longer applies. Remove or replace it and try again.',
      errorPriceChanged: 'A price changed. Review the new total before confirming.',
      errorGeneric: 'We could not place your order. Please try again.',
      subtotal: 'Subtotal',
      discount: 'Discount',
      shipping: 'Shipping',
      tax: 'Tax',
      total: 'Total',
      notCalculated: 'Calculated later',
      itemsTitle: 'Items',
      quantityLabel: 'Quantity',
    },
    orders: {
      successTitle: 'We have your order',
      successBody: 'Your order is saved and we will reach you on the details below.',
      nextStepsTitle: 'What happens next',
      nextStepsBody: 'We will contact you to confirm the order and arrange payment and delivery.',
      saveLinkNotice: 'Keep this link to check on your order later.',
      orderNumber: 'Order number',
      placedAt: 'Placed',
      viewOrder: 'View order',
      myOrders: 'My orders',
      noOrders: 'No orders yet',
      noOrdersBody: 'Your first order will appear here once you place it.',
      date: 'Date',
      total: 'Total',
      items: 'Items',
      itemsCount: '{count} items',
      view: 'View',
      backToOrders: 'Back to orders',
      contactDetails: 'Contact details',
      shippingAddress: 'Shipping address',
      itemsTitle: 'Items',
      timeline: 'Order history',
      orderNote: 'Order note',
      notFound: 'Order not found',
      notFoundBody: 'Check the link or the order number.',
      cancel: 'Cancel order',
      cancelConfirmTitle: 'Cancel this order?',
      cancelConfirmBody:
        'The order will be cancelled and its stock returned. This cannot be undone.',
      cancelConfirm: 'Cancel the order',
      cancelled: 'Order cancelled',
      cancelFailed: 'We could not cancel this order',
      statusLabel: 'Order status',
      paymentLabel: 'Payment',
      fulfillmentLabel: 'Fulfillment',
      statusPendingPayment: 'Awaiting payment',
      statusPendingManual: 'Awaiting confirmation',
      statusConfirmed: 'Confirmed',
      statusProcessing: 'Processing',
      statusCompleted: 'Completed',
      statusCancelled: 'Cancelled',
      paymentUnpaid: 'Unpaid',
      paymentPending: 'Processing',
      paymentPaid: 'Paid',
      paymentFailed: 'Failed',
      paymentRefunded: 'Refunded',
      paymentPartiallyRefunded: 'Partially refunded',
      fulfillmentUnfulfilled: 'Not shipped',
      fulfillmentProcessing: 'Preparing',
      fulfillmentShipped: 'Shipped',
      fulfillmentDelivered: 'Delivered',
      fulfillmentCancelled: 'Cancelled',
      eventCreated: 'Order placed',
      eventOrderStatus: 'Order status changed',
      eventPaymentStatus: 'Payment status changed',
      eventFulfillmentStatus: 'Fulfillment status changed',
      eventNote: 'Note',
      systemActor: 'System',
    },
    account: {
      emailLabel: 'Email',
      emailPlaceholder: 'name@example.com',
      passwordLabel: 'Password',
      passwordPlaceholder: '••••••••',
      passwordConfirmationLabel: 'Confirm password',
      nameLabel: 'Full name',
      phoneLabel: 'Mobile number',
      optional: 'optional',
      showPassword: 'Show password',
      hidePassword: 'Hide password',

      loginTitle: 'Sign in',
      loginSubtitle: 'Sign in to track your orders and manage your account.',
      signIn: 'Sign in',
      signingIn: 'Signing in…',
      noAccountYet: 'Don’t have an account?',
      registerLink: 'Create one',
      forgotPasswordLink: 'Forgot your password?',
      errorValidation: 'Check your email and password.',
      errorRateLimited: 'Too many attempts. Wait a moment and try again.',
      errorInvalidCredentials: 'Incorrect email or password.',

      registerTitle: 'Create your account',
      registerSubtitle: 'Create an account to track your orders easily every time.',
      createAccount: 'Create account',
      creatingAccount: 'Creating your account…',
      alreadyHaveAccount: 'Already have an account?',
      signInLink: 'Sign in',
      errorEmailTaken: 'That email is already registered. Try signing in instead.',
      errorPasswordMismatch: 'Passwords do not match.',
      errorGeneric: 'We could not complete that. Please try again.',
      passwordHint: 'At least 8 characters, with a letter and a number.',

      forgotPasswordTitle: 'Forgot password',
      forgotPasswordSubtitle: 'Enter your email and we will send you a reset link.',
      forgotPasswordSubmit: 'Send reset link',
      forgotPasswordSubmitting: 'Sending…',
      forgotPasswordSuccessTitle: 'Check your email',
      forgotPasswordSuccessBody:
        'If an account exists for that email, a password reset link is on its way.',
      backToLogin: 'Back to sign in',
      resetPasswordTitle: 'Reset your password',
      resetPasswordSubtitle: 'Choose a new password for your account.',
      resetPasswordSubmit: 'Save new password',
      resetPasswordSubmitting: 'Saving…',
      resetPasswordSuccessTitle: 'Password changed',
      resetPasswordSuccessBody: 'You can now sign in with your new password.',
      resetPasswordErrorInvalid: 'This reset link is not valid.',
      resetPasswordErrorExpired: 'This reset link has expired. Request a new one.',
      resetPasswordErrorUsed: 'This reset link was already used. Request a new one.',
      goToLogin: 'Go to sign in',

      verifyEmailTitle: 'Verify your email',
      verifyEmailSuccessTitle: 'Your email is verified',
      verifyEmailSuccessBody: 'Thanks for confirming your email address.',
      verifyEmailErrorInvalid: 'This verification link is not valid.',
      verifyEmailErrorExpired:
        'This verification link has expired. Request a new one from your account.',
      verifyEmailErrorUsed: 'This verification link was already used.',
      verifyEmailPendingNotice: 'Your email address is not verified yet.',
      resendVerification: 'Resend verification link',
      resendingVerification: 'Sending…',
      resendVerificationSuccess: 'A new verification link is on its way.',

      accountTitle: 'My account',
      navOverview: 'Overview',
      navProfile: 'Profile',
      navOrders: 'My orders',
      navBodyProfile: 'Fit profile',
      navSignOut: 'Sign out',
      signingOut: 'Signing out…',
      welcomeBack: 'Welcome back, {name}',
      overviewOrdersTitle: 'Your orders',
      overviewOrdersBody: 'Review your past orders and track their status.',
      overviewOrdersCta: 'View orders',
      overviewProfileTitle: 'Your profile',
      overviewProfileBody: 'Update your name and mobile number.',
      overviewProfileCta: 'Edit profile',

      profileFormTitle: 'Personal information',
      emailReadOnlyNote: 'Email cannot be changed here yet.',
      saveChanges: 'Save changes',
      saving: 'Saving…',
      profileUpdated: 'Your profile has been updated',
      profileValidationError: 'Check the values you entered.',
      profileError: 'We could not save your changes. Please try again.',
      sessionExpired: 'Your session has expired. Please sign in again.',
      unauthorized: 'Sign in to view this page.',
    },

    bodyProfile: {
      title: 'Fit profile',
      intro:
        'Add your measurements once and we will help you pick the right size. They are yours alone — edit or delete them any time.',
      privacyNote:
        'We never ask for your photo or guess anything about how you look — everything here is what you choose.',
      requiredMark: 'Required',
      optionalMark: 'Optional',
      units: { cm: 'cm', kg: 'kg' },
      sections: {
        basic: {
          title: 'Basic information',
          description: 'So we can show you the right cuts first.',
        },
        measurements: {
          title: 'Measurements',
          description:
            'Use a soft tape measure and stand naturally. The required fields are enough to start; the rest make suggestions more precise.',
          required: 'The essentials',
          optional: 'For a closer fit',
        },
        appearance: {
          title: 'Appearance',
          description: 'Entirely optional — it only changes your illustration, never your sizes.',
        },
      },
      fitPreference: {
        label: 'Preferred fit',
        help: 'Sets how much room we allow when recommending a size — it never changes your measurements.',
        options: { SLIM: 'Slim', REGULAR: 'Regular', RELAXED: 'Relaxed' },
      },
      gender: {
        label: 'Gender',
        help: 'Helps us show you the right cuts first. It is your choice — we never infer it.',
        options: {
          FEMALE: 'Female',
          MALE: 'Male',
          OTHER: 'Other',
          PREFER_NOT_TO_SAY: 'Prefer not to say',
        },
      },
      measurements: {
        heightCm: { label: 'Height', help: 'Stand straight against a wall, without shoes.' },
        weightKg: {
          label: 'Weight',
          help: 'Your usual weight. Used as context only — never to pick a size on its own.',
        },
        waistCm: {
          label: 'Waist',
          help: 'Measure around your natural waist, the narrowest part, without pulling the tape tight.',
        },
        chestCm: {
          label: 'Chest',
          help: 'Around the fullest part of your chest, under your arms, tape level.',
        },
        hipCm: { label: 'Hips', help: 'Around the fullest part of your hips, feet together.' },
        shoulderCm: {
          label: 'Shoulder width',
          help: 'Across your back, from one shoulder point to the other.',
        },
        inseamCm: { label: 'Inseam', help: 'From the top of your inner thigh down to your ankle.' },
        sleeveLengthCm: {
          label: 'Sleeve length',
          help: 'From your shoulder point down to your wrist, arm relaxed.',
        },
        neckCm: { label: 'Neck', help: 'Around the base of your neck, one finger under the tape.' },
      },
      guide: {
        open: 'How to measure',
        title: 'How to take your measurements',
        intro:
          'Two minutes and a soft tape measure are all you need. A few tips that apply to every measurement:',
        tips: [
          'Measure over light clothing, or none if you can.',
          'Keep the tape level and snug against your body, without squeezing.',
          'Stand naturally and breathe normally — don’t pull your stomach in.',
          'Between two numbers? Take the larger one; you can change it later.',
        ],
        close: 'Got it',
      },
      appearance: {
        notSpecified: 'Not specified',
        skinTone: {
          label: 'Skin tone',
          options: {
            LIGHT: 'Light',
            MEDIUM_LIGHT: 'Medium light',
            MEDIUM: 'Medium',
            MEDIUM_DARK: 'Medium dark',
            DARK: 'Dark',
          },
        },
        hairStyle: {
          label: 'Hair style',
          options: {
            SHORT: 'Short',
            MEDIUM: 'Medium',
            LONG: 'Long',
            STRAIGHT: 'Straight',
            WAVY: 'Wavy',
            CURLY: 'Curly',
            BUZZ: 'Buzz cut',
            COVERED: 'Hijab / covered',
          },
        },
        hairColor: {
          label: 'Hair colour',
          coveredNote: 'No hair colour needed with a head covering.',
          options: {
            BLACK: 'Black',
            DARK_BROWN: 'Dark brown',
            BROWN: 'Brown',
            LIGHT_BROWN: 'Light brown',
            BLONDE: 'Blonde',
            RED: 'Red',
            GRAY: 'Grey',
            WHITE: 'White',
          },
        },
        facialHair: {
          label: 'Facial hair',
          options: {
            NONE: 'None',
            SHORT_BEARD: 'Short beard',
            LONG_BEARD: 'Long beard',
            MUSTACHE: 'Moustache',
          },
        },
        glasses: {
          label: 'I wear glasses',
          style: {
            label: 'Glasses shape',
            options: {
              ROUND: 'Round',
              RECTANGULAR: 'Rectangular',
              AVIATOR: 'Aviator',
              CAT_EYE: 'Cat-eye',
            },
          },
          frameColor: {
            label: 'Frame colour',
            options: {
              BLACK: 'Black',
              BROWN: 'Brown',
              GOLD: 'Gold',
              SILVER: 'Silver',
              CLEAR: 'Clear',
            },
          },
        },
      },
      preview: {
        title: 'Your illustration',
        caption: 'Drawn from your measurements and choices — an illustration, not a photo.',
        heightUnset: 'Add your height to draw the figure to scale.',
        glasses: 'glasses',
        bodyShape: {
          STRAIGHT: 'straight proportions',
          DEFINED_WAIST: 'defined waist',
          HIPS_WIDER: 'hips wider than chest',
          CHEST_WIDER: 'chest wider than hips',
          WAIST_WIDEST: 'waist wider than chest and hips',
        },
      },
      completion: {
        title: 'Profile completeness',
        percent: 'Your profile is {percent}% complete',
        complete: 'Your profile is complete. Nice work!',
        sectionDone: 'Complete',
        sectionProgress: '{done} of {total}',
        requiredMissing: 'Fill in the required fields to save.',
        next: {
          gender: 'Choose a gender option so we can show you the right cuts.',
          heightCm: 'Add your height — required to save.',
          weightKg: 'Add your weight — required to save.',
          waistCm: 'Add your waist — required to save.',
          hipCm: 'Add your hip measurement to improve size recommendations.',
          chestCm: 'Add your chest measurement for better-fitting tops.',
          inseamCm: 'Add your inseam for trousers of the right length.',
          shoulderCm: 'Add your shoulder width for jackets and shirts.',
          sleeveLengthCm: 'Add your sleeve length for long sleeves that fit.',
          neckCm: 'Add your neck size for shirt collars.',
          skinTone: 'Pick a skin tone for your illustration.',
          hairStyle: 'Pick a hair style for your illustration.',
          hairColor: 'Pick a hair colour for your illustration.',
        },
      },
      errors: {
        required: 'This field is required.',
        invalid_number: 'Enter a number only, like 88 or 88.5.',
        too_small: 'That looks too small — enter at least {min} {unit}.',
        too_large: 'That looks too large — enter at most {max} {unit}.',
        invalid_option: 'Choose one of the options.',
        summary: 'Check the highlighted fields, then save again.',
        stale: 'Your profile was changed in another window. Refresh the page and try again.',
        sessionExpired: 'Your session has expired. Please sign in again.',
        generic: 'We could not save right now. Please try again.',
      },
      save: 'Save profile',
      saving: 'Saving…',
      saved: 'Fit profile saved',
      unsaved: 'Unsaved changes',
      delete: {
        button: 'Delete fit profile',
        title: 'Delete your fit profile?',
        description:
          'Your measurements and appearance settings will be deleted permanently. You can create a new profile any time.',
        confirm: 'Delete permanently',
        cancel: 'Cancel',
        deleting: 'Deleting…',
        deleted: 'Fit profile deleted',
      },
    },

    sizing: {
      title: 'Size & fit',
      chartRecommended: 'Recommended for you',
      recommended: 'Your recommended size: {size}',
      loading: 'Working out your size…',
      confidence: { HIGH: 'High confidence', MEDIUM: 'Medium confidence', LOW: 'Low confidence' },
      confidenceHelp: {
        HIGH: 'Your measurements clearly match this size.',
        MEDIUM: 'A good fit, with small differences in some measurements.',
        LOW: 'The closest size available, but not an exact match.',
      },
      why: 'Why this size?',
      fitLabel: 'Fit',
      fitOptions: { SLIM: 'Slim', REGULAR: 'Regular', RELAXED: 'Relaxed' },
      fits: {
        fits: 'matches',
        snug: 'slightly snug',
        loose: 'slightly loose',
        too_tight: 'tight',
        too_loose: 'loose',
      },
      reasonMeasurement: '{measurement} {fit}',
      reasonFit: {
        SLIM: 'Slim fit preference',
        REGULAR: 'Regular fit preference',
        RELAXED: 'Relaxed fit preference',
      },
      reasonLengthFromHeight: 'Length estimated from your height',
      reasonBetweenSizes: 'You are between sizes — {size} is close too',
      reasonLimited: 'Add your {measurements} for a more precise result',
      measurementNames: {
        chestCm: 'Chest',
        waistCm: 'Waist',
        hipCm: 'Hip',
        shoulderCm: 'Shoulder',
        sleeveCm: 'Sleeve',
        inseamCm: 'Inseam',
        lengthCm: 'Length',
        neckCm: 'Neck',
      },
      profileMeasurementNames: {
        heightCm: 'height',
        waistCm: 'waist measurement',
        chestCm: 'chest measurement',
        hipCm: 'hip measurement',
        shoulderCm: 'shoulder width',
        inseamCm: 'inseam',
        sleeveLengthCm: 'sleeve length',
        neckCm: 'neck measurement',
      },
      listJoin: ' and ',
      alternatives: 'Alternatives',
      alternative: { smaller: '{size} — closer fit', larger: '{size} — looser fit' },
      selectSize: 'Select size {size}',
      selectedSize: 'Size {size} selected',
      insufficient: 'Add your {measurements} to get a recommendation for {item}.',
      items: {
        T_SHIRT: 'this T-shirt',
        SHIRT: 'this shirt',
        HOODIE: 'this hoodie',
        SWEATER: 'this sweater',
        JACKET: 'this jacket',
        CARDIGAN: 'this cardigan',
        DRESS: 'this dress',
        ABAYA: 'this abaya',
        THOBE: 'this thobe',
        JEANS: 'these jeans',
        TROUSERS: 'these trousers',
        SHORTS: 'these shorts',
        SKIRT: 'this skirt',
      },
      updateProfile: 'Update your fit profile',
      noMatch: {
        above: 'Your measurements are above the largest size this product comes in.',
        below: 'Your measurements are below the smallest size this product comes in.',
        mixed: 'None of this product’s sizes fits your measurements well.',
      },
      noMatchHelp: 'Check the size chart, or try another fit.',
      signedOut: {
        title: 'Know your size before you buy',
        body: 'Create your fit profile once, and we will recommend the right size for every piece.',
        cta: 'Create your fit profile',
      },
      noProfile: {
        title: 'Know your size before you buy',
        body: 'Add your measurements once, and we will recommend the right size for every piece.',
        cta: 'Create your fit profile',
      },
      error: 'We could not work out your size just now. The size chart can still help.',
      howItWorks: 'How is this worked out?',
      howItWorksBody:
        'We compare each size’s measurements with yours, allowing the room your preferred fit calls for. Confidence reflects how complete and close the measurements are — it is a clear rule, not a statistic.',
      privacy:
        'Your size is worked out from your profile on our servers; your measurements are never shown here.',
      sizeGuide: {
        open: 'Size chart',
        title: 'Size chart',
        description: 'The garment’s own measurements, laid flat, in centimetres.',
        size: 'Size',
        unit: 'cm',
      },
    },

    email: {
      greeting: 'Hi {name},',
      greetingNoName: 'Hi,',
      fallbackNotice: "If the button doesn't work, copy and paste this link into your browser:",
      footer: '© {year} {store}. All rights reserved.',
      automatedNotice: 'This is an automated message — please do not reply.',

      verificationSubject: 'Verify your email — {store}',
      verificationHeading: 'Verify your email address',
      verificationBody:
        'Thanks for creating a {store} account. To finish setting it up, please confirm this is your email address.',
      verificationCta: 'Verify email',
      verificationExpiry: 'This link expires in 24 hours.',
      verificationIgnore: "If you didn't create this account, you can safely ignore this email.",

      passwordResetSubject: 'Reset your password — {store}',
      passwordResetHeading: 'Reset your password',
      passwordResetBody: 'We received a request to reset the password for your {store} account.',
      passwordResetCta: 'Reset password',
      passwordResetExpiry: 'This link expires in 1 hour.',
      passwordResetIgnore:
        "If you didn't request this, you can safely ignore this email — your password will not change.",
    },
  },
} as const satisfies Record<Locale, unknown>;

export function getDictionary(locale: Locale) {
  return dictionary[locale];
}
